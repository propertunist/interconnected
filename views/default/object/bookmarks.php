...
$link = elgg_view('output/url', array('href' => $url, 'text' => elgg_get_excerpt($url, 120)));
$tags = elgg_view('output/tags', array('tags' => $bookmark->tags));
$entity_url = elgg_view('output/url', array('href' => $bookmark->getURL(), 
                                            'text' => $title,
                                            'is_trusted' => TRUE));
...
$subtitle = "$author_text $date $comments_link $categories $tags";
...
if ($full && !elgg_in_context('gallery')) {
    ...
    $bookmark_icon =  elgg_view('icon/object/bookmark', array(
        'entity' => $bookmark,
        'size' => 'large',
    ));

    if ($bookmark_icon)
        $bookmark_icon = '<div class="elgg-bookmark-icon-holder">' . $bookmark_icon . '</div>';
    else
        $bookmark_icon = elgg_view_icon('push-pin-alt');

	$body = <<<HTML
<div class="bookmark elgg-content scrape-this mts">
...
} elseif (elgg_in_context('gallery')) 
{
	...
	echo <<<HTML
<div class="bookmarks-gallery-item compact-gallery-item">
    $bookmark_icon
	<h3>$entity_url</h3>
...
HTML;
	...
	echo elgg_view_image_block($bookmark_icon, $body);
}
